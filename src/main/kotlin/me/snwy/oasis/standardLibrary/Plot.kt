...
val plot = Module("Plot") { env, interpreter ->
    val plot = OasisPrototype(base, -1, interpreter).apply {
        set(
            "function",
            KotlinFunction4<Unit, String, Double, Double, OasisCallable> { interpreter, x, y, precision, z ->
                val xData = 0.until(y.toInt() * precision.toInt()).map { it.toDouble() / precision }
                val yData = xData.map {
                    z.call(interpreter, listOf(it)) as? Double ?: throw RuntimeError(
                        line,
                        "Plot function must return numeric value"
                    )
                }
        ...
        set(
            "functionWithRange",
            KotlinFunction5<Unit, String, Double, Double, Double, OasisCallable> { interpreter, x, range, y, precision, z ->
