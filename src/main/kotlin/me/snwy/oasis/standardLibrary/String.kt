...
val string = Module("string") { it, interpreter ->
    ...
    string.set(
        "substring",
        KotlinFunction3<String, String, Double, Double> { x, y, z -> x.substring(y.toInt(), z.toInt()) })
    ...
    string.set(
        "split",
        KotlinFunction2<ArrayList<Any?>, String, String> { x, y -> x.split(y).filter { it != y } as ArrayList<Any?> })
    ...
    string.set(
        "toCharArray",
        KotlinFunction1<ArrayList<Any?>, String> { z -> z.toCharArray().toList() as ArrayList<Any?> })
